...
from agent_workflow.integrations import connectors, github
from agent_workflow import rules
...
class IntegrationTests(WorkspaceTest):
    ...
    def test_github_paginates_threads_and_nested_comments(self):
        ...
        with patch("agent_workflow.integrations.github.run", side_effect=response):
    ...
    def test_bot_ready_transition_restores_draft_even_on_fetch_failure(self):
        ...
        with patch("agent_workflow.integrations.github.run", side_effect=command), \
             patch("agent_workflow.integrations.github.comments", side_effect=WorkflowError("fixture failure")):
