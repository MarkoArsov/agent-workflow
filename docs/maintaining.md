...
| One command's behavior | A project skill override: `scorebook skill copy NAME` |
| A new command | A new project skill: `scorebook skill new NAME` |
| Detailed guidance for one kind of work | `.scorebook/references/`, named by the plans that need it |
...
2. **Find** the file that already owns that part of the workflow. `scorebook inspect` and `skill resolve` show the effective sources.
...
6. **Refresh** host discovery with `scorebook refresh` if you added or renamed skills, and apply the reviewed proposal.
...
Keep rules as narrow as possible, by path and repository. Record new examples with `scorebook rule record RULE --detail "…"`; at the rule's `review_after` threshold, you get a proposal for a corrective review, never an automatic promotion to blocking.
