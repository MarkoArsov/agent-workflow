...
!!! summary "In one minute"
    - Most of Stagecoach is Python and shell. Verification, status, watching, and delivery make no model calls.
...
Stagecoach writes the repeatable procedure down once as skills and scripts, then spends model tokens only on judgement.
...
- Stagecoach ships no model catalog. Setup records the models you choose.
