...
Invoke skills as `aw-specify` in standalone installs, or `agent-workflow:specify` in the native Claude plugin.
...
agent-workflow run ai-plans/csv-export/pipeline.json --dry-run
~~~
...
For a project install, use `.agent-workflow/bin/agent-workflow`.
...
~~~sh
agent-workflow status csv-export
agent-workflow watch csv-export --seconds 120
~~~
...
~~~sh
agent-workflow answer csv-export --file answer.txt
~~~
