...
def parser():
    p = argparse.ArgumentParser(prog="scorebook", description="One workflow, customized for each project.")
...
def execute(args):
    if args.command in {"update", "uninstall", "doctor"}:
        ...
        if args.backend == "claude-plugin":
            ...
            if verb != "list":
                argv += ["scorebook@scorebook", "--scope", "user" if args.global_install else "project"]
            if args.dry_run:
