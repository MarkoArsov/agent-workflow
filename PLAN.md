...
```json
{
  ...
  "planning": {
    "directory": "ai-plans",
    "evidence_directory": ".scorebook/local/evidence"
  },
  "git": {
    "branch_template": "feature/{task}",
    "commit_style": "imperative",
    "worktree_directory": ".scorebook/local/worktrees"
  },
  "delivery": {
    "pull_requests": "draft",
  ...
  "extensions": {
    "skills": ".scorebook/skills",
    "rules": ".scorebook/rules",
    "references": ".scorebook/references",
    "connectors": ".scorebook/connectors",
    "stages": ".scorebook/stages.json"
  }
}
```
...
for each skill and show its origin in `scorebook inspect`.
...
```sh
curl -fsSL https://raw.githubusercontent.com/<owner>/scorebook/<ref>/install.sh | sh -s -- --global
curl -fsSL https://raw.githubusercontent.com/<owner>/scorebook/<ref>/install.sh | sh -s -- --project /path/to/project
```
...
- Global: versioned payload under `~/.local/share/scorebook/`, a user CLI shim,
  and installer-owned host discovery entries. Respect supported XDG/config overrides.
- Project: payload under `<project>/.scorebook/runtime/`, with a local launcher
...
Global and project discovery wrappers use the same resolver. Use namespaced `sb-*`
...
Provide `scorebook update`, `uninstall`, `inspect`, and `doctor` with explicit
...
3. Adding `git@github-personal:<owner>/scorebook.git`.
