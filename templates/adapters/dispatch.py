...
for parent in [start, *start.parents]:
    ref = parent / ".scorebook/project-ref.json"
    profile = parent / ".scorebook/project.json"
    if ref.is_file():
...
if project is None:
    try:
        ...
        for parent in [original, *original.parents]:
            if (parent / ".scorebook/project.json").is_file():
                project = parent
                break
...
version = None
if project:
    data = json.loads((project / ".scorebook/project.json").read_text())
    version = data["package"]["version"]
global_root = Path(os.environ.get("XDG_DATA_HOME", str(Path.home() / ".local/share"))) / "scorebook"
candidates = []
if project and version:
    candidates.append(project / ".scorebook/runtime/versions" / version)
if version:
    candidates.append(global_root / "versions" / version)
active = global_root / "active.json"
if not version:
    for parent in [start, *start.parents]:
        local_active = parent / ".scorebook/runtime/active.json"
...
if os.environ.get("SCOREBOOK_PACKAGE"):
    candidates.append(Path(os.environ["SCOREBOOK_PACKAGE"]))
for candidate in candidates:
    ...
    if marker.is_file() and (not version or json.loads(marker.read_text())["version"] == version):
        ...
        from scorebook.extensions import resolve_skill
