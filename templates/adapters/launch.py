...
for parent in search:
    ...
    marker = target / ".scorebook/project.json"
    if marker.is_file():
        profile = json.loads(marker.read_text()); project = target; break
if profile:
    version = profile["package"]["version"]
    candidates += [project / ".scorebook/runtime/versions" / version, global_root / "versions" / version]
else:
    for root in [LOCAL_RUNTIME, *(p / ".scorebook/runtime" for p in search), global_root]:
...
for candidate in candidates:
    ...
    if marker.is_file() and (not profile or json.loads(marker.read_text())["version"] == profile["package"]["version"]):
        ...
        from scorebook.cli import main
