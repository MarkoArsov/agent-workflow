...
Stagecoach can open a draft pull request. It never approves or merges one.
...
Stagecoach also admits only one runner per project, held by an operating-system lock, so concurrent runs can't fight over the same files.
